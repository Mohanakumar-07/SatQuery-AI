...
- Plan version: Freeze Candidate 1.2
- Review status: Revised after evidence semantics, geospatial measurement, routing-state, asynchronous execution, visualization, alignment, confidence, SAR-FuseSeg, feasibility-gate, and licence review
...
| Background jobs | Redis + RQ mandatory for model inference |
...
| ChangeNet | ChangeFormer V6 pretrained baseline | Bi-temporal binary change mask and binary change facts |
...
- Reproject T1 and T2 to a common CRS, grid, resolution, and extent.
- Validate residual spatial alignment before inference.
- Reject the pair when residual misalignment exceeds a validated threshold.
- Allow only a minor, explicitly tested alignment correction; do not promise arbitrary automatic satellite-image co-registration.
...
- Validate residual spatial alignment before fusion.
- Reject the pair when alignment quality is outside the validated tolerance.
...
### 7.0 Asynchronous inference architecture

All model inference must run outside the FastAPI request process.
...
- one GPU worker;
- one analysis running at a time;
- SatVLM remains loaded when memory permits;
- ChangeNet and SAR-FuseSeg may be loaded on demand or in separate processes;
- models may be unloaded or process-isolated when necessary to avoid GPU-memory exhaustion;
- FastAPI returns the job/analysis ID immediately after enqueueing;
- React polls job status until completion, failure, or clarification.
...
### 7.5 Needs-clarification response

Automatic routing must have an explicit clarification state for ambiguous input roles.
...
The user may identify file roles, dates, or modality information when the system cannot safely resolve them. The user must never be asked to select SatVLM, ChangeNet, or SAR-FuseSeg.
...
```json
{
  ...
  "answer": "Change was detected in three regions in the north-east portion of the image.",
  "evidence": {
    "area_value": 18450,
    "area_unit": "m2",
    ...
    "overlay": {
      "format": "png",
      "url": "/api/v1/analyses/analysis-001/artifacts/change-mask.png",
      "bounds": [[12.91, 77.51], [12.98, 77.60]]
    },
    "geojson_url": "/api/v1/analyses/analysis-001/artifacts/change-regions.geojson"
  },
  "confidence": {
    "decision": "accepted",
    "specialists": [
...
The validation layer distinguishes **georeferenced** imagery from **non-georeferenced benchmark/demo imagery**. Geographic coordinates and square-metre area may only be returned when reliable georeferencing and a suitable measurement CRS are available.
...
### 8.5 Area and coordinate contract

For georeferenced rasters:

1. Preserve the source CRS and raster transform.
2. Select an appropriate projected measurement CRS, normally a suitable local UTM CRS or an appropriate equal-area CRS.
3. Calculate polygon area in the measurement CRS, not directly in latitude/longitude degrees.
4. Return the measurement CRS with the result.
...
For PNG/JPEG or any raster without reliable georeferencing:
...
Never invent latitude/longitude, projected coordinates, or square-metre area for an ungeoreferenced image.
...
- Run **binary change** inference.
- Restore the mask to original image coordinates.
- Remove noise only through a documented, validated post-processing policy.
- Extract connected changed regions.
- Calculate area and percentage only when georeferencing and a valid measurement CRS are available.
- Produce structured **binary change facts** before language generation.

ChangeNet by itself is limited to:

- whether change occurred;
- where change occurred;
- number of changed regions;
- changed area when measurable.

ChangeNet must **not** independently claim that a changed region became built-up, water, vegetation, or another semantic class. A later extension may run a land-cover model on T1/T2 or the changed regions to infer semantic transitions, but that is outside the first MVP.
...
For the first reproducible SAR-FuseSeg implementation, freeze:

- Optical input: RGB or the explicitly selected Sentinel-2 bands used by the training dataset.
- SAR input: VV and VH.
- Optical encoder: lightweight ResNet-18.
- SAR encoder: lightweight ResNet-18.
- Fusion: multi-scale feature concatenation.
- Decoder: U-Net-style decoder.
- Output classes: built-up, water, vegetation, other.
- Ignore index: one explicit dataset-mapped ignore index for invalid/unknown pixels.
- Loss: weighted cross-entropy plus Dice loss.
- Class weighting: computed from the frozen training split only.
- Augmentation: paired spatial transforms applied identically to optical and SAR; modality-specific intensity augmentation only where validated.
- Tile size: 256×256 initially; change only after the baseline is reproducible.

Any later architecture change must be recorded as an experiment rather than silently changing the MVP baseline.
...
### 11.0 SatVLM confidence semantics

SatVLM confidence must not be represented as a generic percentage for arbitrary generated text.

Use:

- **Closed-answer VQA:** calibrated answer probability or a validated answer-score mapping.
- **Caption / free-text factuality:** evidence coverage and evidence consistency.
- **Final composed answer:** claim-validation status plus evidence coverage.
- **Missing or contradictory evidence:** warning or abstention.
...
Do not display a percentage labelled "AI confidence" for free-text generation unless its meaning and calibration procedure are explicitly defined and validated.
...
## 12.0 Web-compatible evidence artifacts

The backend must never return a raw model tensor directly to the map.

For the MVP:

- georeferenced raster masks are rendered as PNG overlays with geographic bounds;
- extracted regions are rendered as GeoJSON polygons;
- ungeoreferenced benchmark/demo masks use pixel-space overlays and do not expose geographic coordinates.
...
```json
{
  ...
  "regions_geojson_url": "/api/v1/analyses/analysis-001/artifacts/change-regions.geojson"
}
```

React Leaflet renders the PNG overlay using its bounds and GeoJSON for vector evidence. A full tile server is not required for the MVP.
...
VRSBench, RSVQA, CDVQA and hidden ISRO/SAC evaluation data are evaluation-only. They must not be used for training, QLoRA adaptation, prompt selection, hyperparameter tuning, checkpoint selection or threshold calibration.
...
All six core gates below are mandatory:

- React → FastAPI upload works.
- GeoTIFF validation works.
- Qwen inference works on one remote-sensing example.
- ChangeFormer inference works on one temporal pair.
- An aligned optical-SAR pair loads correctly.
- A geographic or pixel-space mask appears in Leaflet according to the input georeferencing state.

If any core gate fails, stop and fix it or explicitly reduce scope before proceeding. The team may not proceed merely because a majority of unrelated checks pass.

### Model and Dataset Licence Gate

No model checkpoint or dataset is treated as production-ready until its exact terms have been recorded.

For every model, record separately:

- model source;
- model version;
- code repository and code licence;
- checkpoint source and checkpoint licence/terms;
- checksum;
...
- version;
- dataset licence;
- source-imagery restrictions;
- redistribution restrictions;
- attribution requirements.

Record the selected Qwen2.5-VL-7B-Instruct artifact as **Apache-2.0**, subject to verification of the exact downloaded artifact.

ChangeFormer remains subject to an explicit review of both repository/code terms and checkpoint terms before packaging or redistribution.

Create a licence manifest before the first production packaging step.
...
- Single-SAR VQA/captioning uses the documented VV/VH visualization adapter.
...
### Semantic Change Extension (Post-MVP)

The MVP ChangeNet path is binary change detection only.

If later evaluation or requirements demand statements such as "vegetation changed to built-up" or "water became built-up", add a separate land-cover model on T1/T2 or on detected changed regions and derive semantic transitions from those class predictions.

Such semantic transitions must not be attributed to ChangeNet alone.

### Final Freeze Checklist

Before implementation begins:

- [ ] ChangeNet is documented as binary change detection only.
- [ ] Georeferenced and non-georeferenced result contracts are distinct.
- [ ] Square-metre area requires a valid measurement CRS.
- [ ] `needs_clarification` is an explicit API state.
- [ ] Redis + GPU worker execution is mandatory for model inference.
- [ ] MVP GPU concurrency is one analysis at a time.
- [ ] Leaflet receives PNG overlays + bounds and GeoJSON, not raw tensors.
- [ ] Temporal and optical-SAR paths validate residual alignment instead of promising arbitrary automatic co-registration.
- [ ] SatVLM free-text confidence is represented through evidence coverage / claim validation unless a validated correctness score exists.
- [ ] SAR-FuseSeg baseline inputs, architecture, loss, classes, ignore index, augmentation, and tile size are frozen.
- [ ] Training, calibration, and evaluation data are isolated.
- [ ] Model code and checkpoint licences are reviewed separately.
- [ ] All six core feasibility gates pass.
- [ ] Official SIH judging criteria and weights are verified before performance optimization.
