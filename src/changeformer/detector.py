"""
detector.py -- Production inference engine for ChangeFormer V6 (OSCD Champion).
...
_possible_checkpoints = [
    ...
    _repo_root / "artifacts" / "checkpoints" / "changeformer" / "best_ckpt.pt",
    _repo_root / "experiments" / "oscd" / "v3_1_controlled_sampler" / "checkpoints" / "best.pt",
    Path("E:/SatQuery-Models-Dev/changenet/experiments/oscd/v3_1_controlled_sampler/checkpoints/best.pt"),
]
DEFAULT_CHECKPOINT = next((p for p in _possible_checkpoints if p.is_file()), _possible_checkpoints[0])
...
class ChangeDetector:
    ...
    def save_visualization(
        ...
        axes[2].set_title(f"Confidence Heatmap (tau={thr:.2f})", fontsize=12, fontweight="bold")
        axes[2].axis("off")
